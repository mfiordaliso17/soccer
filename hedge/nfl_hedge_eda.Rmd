...
# create range for list
interval_amt <- 50

und_filter_range <- seq(100, 700, by = interval_amt)
fav_filter_range <- seq(-700, -100, by = interval_amt)

# create lists for underdog and favorite
und_lead_chg_range <-  lapply(und_filter_range, function(x) {
  
    
  # lead change function sourced
  lead_chg_list <- lead_chg_qtr_summary_fn(odds_df %>%
                            filter(und_odd <= x), "underdog") %>% 
    mutate(odd = x)
...
fav_lead_chg_range <- lapply(fav_filter_range, function(x) {
  
    
  # lead change function sourced
  lead_chg_list <- lead_chg_qtr_summary_fn(odds_df %>%
                            filter(fav_odd >= x), "favorite") %>% 
    mutate(odd = x)
...
lead_chg_matrix <- bind_rows(do.call(rbind, und_lead_chg_range),
                             do.call(rbind, fav_lead_chg_range))

sample_size <- lead_chg_matrix %>% 
  group_by(type, odd) %>% 
...
one_qtr_min <- lead_chg_matrix %>% 
  filter(quarters_led > 0) %>% 
  group_by(type, odd) %>% 
  summarize(cum_freq = sum(freq)) %>% 
  ungroup() %>% 
  left_join(sample_size, by = c("type", "odd"))

one_qtr_min %>% 
  ggplot() +
  geom_line(aes(x = odd, y = cum_freq, color = type)) +
  scale_x_continuous(breaks = seq(min(fav_filter_range), max(und_filter_range), by = interval_amt))

```

```{r}
# reevaluate graph as there appears to be significant diminishing decrease for underdog at -500 or greater. if true could be an interesting component to a strategy
